#!/usr/bin/env python3
"""Trace APPSBL past pc 0x8d78 (end of MDDI init) to capture the handoff target:
does it branch/jump to another address (a loader), or fall off? Log all control
transfers after 0x8d78 with the instruction and destination."""
...
st={"n":0,"last":0,"trace":False,"b":0}
def hc(uc,ad,sz,u):
    st["n"]+=1
    if ad==0x8ec: uc.reg_write(UC_ARM_REG_R0,1)
    if ad==0x8d7c: st["trace"]=True
    if st["trace"] and st["last"] and ad!=st["last"]+4 and st["b"]<90:
        b=bytes(uc.mem_read(st["last"],4))
        try: dis=next(md.disasm(b,st["last"])); dd="%s %s"%(dis.mnemonic,dis.op_str)
        except: dd="?"
        print("0x%08x -> 0x%08x  [%s] lr=0x%08x"%(st["last"],ad,dd,uc.reg_read(UC_ARM_REG_LR)))
        st["b"]+=1
        if st["b"]>=90: uc.emu_stop()
    st["last"]=ad
    if st["n"]>3_500_000: uc.emu_stop()
...
print("final pc=0x%08x branches=%d err=%s"%(uc.reg_read(UC_ARM_REG_PC),st["b"],err))