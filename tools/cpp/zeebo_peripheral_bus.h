...
// -------------------------------------------
// Addresses below are transcribed from the OpenZeebo zloader headers that cite
// Qualcomm doc "80-VE113-1 A" for the MSM7200/MSM7201A:
...
// VIC base 0xC0000000, GPT base 0xC0100000. Only the low interrupt word
// (lines 0..31, the STATUS0/EN0/CLEAR0 bank) and GPT0 are modelled; that is the
// honest scope, labelled where behaviour is [model] beyond the register offset.
//
...
// ---- MSM7201A MMIO map (primary source: see file header) ----
enum : uint32_t {
    PB_VIC_BASE          = 0xC0000000u,
    PB_VIC_SIZE          = 0x00001000u,
    PB_GPT_BASE          = 0xC0100000u,
...
struct PeripheralBus {
    ...
    bool mmio_write(uint32_t addr, uint32_t val) {
        if (addr >= PB_VIC_BASE && addr < PB_VIC_BASE + PB_VIC_SIZE) {
            switch (addr - PB_VIC_BASE) {
        ...
        if (addr >= PB_GPT_BASE && addr < PB_GPT_BASE + PB_GPT_SIZE) {
            switch (addr - PB_GPT_BASE) {
