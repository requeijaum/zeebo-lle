...
2. **Passo 14 — Despacho IPC no Iguana Server Loop & Boot do BREW AppMgr** (QW32 concluído, avanço para QW33)
   ...
   - QW28 (estrutura e tipos MsgTag), QW29 (tabela de threads e captura via ExchangeRegisters), QW30 (chaveamento cooperativo em `L4_ThreadSwitch`), QW31 (registro de serviços e buffers em `SystemServiceRegistry`) e QW32 (chaveamento cooperativo em wait `L4_Ipc`) foram CONCLUÍDOS.
   - Bloqueio atual = QW33: Handoff para o processo de espaço de usuário do `AEECShell` / BREW em `0x10137000` / `0x10c874f4`.
   - Handoff para o processo de espaço de usuário do `AEECShell` / BREW em `0x10137000` / `0x10c874f4`.
...
| QW33 | **proposto** | Handoff para o processo de espaço de usuário do `AEECShell` / BREW | médio-alto | transferir execução do Iguana OS para o ponto de entrada do shell BREW (`0x10137000`) |
