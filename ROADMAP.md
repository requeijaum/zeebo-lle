...
| QW41 | **bloqueio aberto** | Core0 executa código AMSS/BREW real a partir de `0x10137000` mas falha em `0x1039322e` | alto | Disassembly confirma que os bytes em `0x1039322e` são válidos em modo **Thumb** (`cbz r4, #0x10393262`) mas o emulador decodificou/executou como **ARM**, produzindo instrução inválida. Suspeita: o handoff via `ExchangeRegisters`/`ThreadStart` não está propagando o T-bit (bit 5 do CPSR) — o Unicorn precisa ser colocado em modo Thumb ao setar o novo PC/IP para essa thread. Próximo passo: TDD que reproduza o `ip` alvo do handoff e valide que o modo Thumb é ativado corretamente antes do primeiro `uc_emu_start` da thread AMSS |
