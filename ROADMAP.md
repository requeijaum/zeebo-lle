...
**QW43 (NOVO bloqueio aberto)**: o loop memcpy em `0xb0400064` NÃO termina — dump do contador `r4` ao vivo (`ZEEBO_DUMP_LOOP2`, temporário/revertido) mostra `r4` partindo de valores pequenos (1,4,3,2,1 nas primeiras iterações) mas depois assumindo um valor gigantesco (~`0xfffcf34a`, ~4 bilhões) e decrescendo ~13/iteração sem nunca zerar em 20M+ iterações/90s de boot. Não é um memcpy legítimo de firmware (nenhuma cópia de dados reais chega a bilhões de bytes) — indica que `r4` foi inicializado com um valor corrompido em algum ponto anterior (possivelmente um cálculo de tamanho/offset errado, subtração que underflowa, ou registrador não inicializado lido de contexto salvo incorretamente). Próximo passo: rastrear de onde vem o valor de `r4` antes da entrada no loop (LR do chamador, contexto de `bl`/`svc` anteriores) para achar o cálculo que gera esse tamanho absurdo. Log adicional: `[MMU/WARN] page @ 0x16000000/.../0x16c00000 EMPTY/UNINITIALIZED` ainda aparece antes — não confirmado como relacionado |
