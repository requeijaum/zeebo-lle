...
**QW43 — desambiguação (b) descartada, hipótese (a) reforçada**: `ZEEBO_WATCH_SLOT` (temporário, revertido) monitorou TODAS as escritas no endereço exato `0xb04152b8` (offset 0x18/índice 6 da tabela) durante o boot inteiro: houve escrita EXPLÍCITA de `value=0x0` (2 ocorrências) — não é memória órfã/não-inicializada por falta de um passo do emulador (descarta hipótese b). O firmware/`ig_naming` grava deliberadamente zero nesse slot, reforçando que é um terminador de tabela intencional por design (hipótese a: bug real do loop consumidor de `ig_naming`, que não trata entrada nula como fim-de-tabela e cai no cálculo genérico de length-prefix, causando o underflow `0xffffffff`). Como esse código pertence ao firmware AMSS original 1.1.2 (não escrito por nós), a correção correta é do lado do EMULADOR: ou (i) o L4/kernel real faria o slot nunca ser alcançado por esse consumidor específico (timing/scheduling diferente do simulado — outro serviço deveria ter registrado algo nesse slot antes do consumo, e nosso `service_registry_`/scheduling cooperativo pode estar entregando o slot cedo demais), ou (ii) há uma condição de guarda antes do loop (comparação de contagem real de entradas, não dependente do valor da entrada) que o emulador não está reproduzindo por falta de outro dado de contexto (ex. um contador de entradas válidas mantido em outro registro/memória que não capturamos ainda). Próximo passo: achar ONDE o valor 6 (quantidade de entradas válidas) deveria ser lido/comparado antes de entrar no loop — provavelmente em memória próxima à tabela (`0xb04152a0-8` ou similar) ou em um registrador salvo por outra chamada anterior do laço externo (`0xb0400014`, ver `sl`/`fp` já mapeados). Nenhum fix aplicado ainda. |
