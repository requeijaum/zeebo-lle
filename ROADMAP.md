...
**QW43 — causa raiz REFINADA (2ª rodada de RE)**: instrumentação adicional (`ZEEBO_DUMP_STRBUF`, revertida) dumpou o conteúdo real do buffer processado no ponto do "byte 0x00": NÃO é uma string ASCII/UTF-8 — é uma TABELA DE PONTEIROS de 32-bit little-endian em `0xb04152a0` (dentro da região `ig_naming`, RAM real via `uc_mem_read`), com entradas como `0xb0401b0c, 0xb0401b14, 0xb0401a00, 0xb04017b4, 0xb04015a4, 0xb0401654, 0x00000000, 0xb04016d4` — todos apontando para outras regiões dentro de `0xb0400000-0xb0402000` (código/dados de `ig_naming`), EXCETO a 7ª entrada (índice 6, offset 0x18 na tabela), que é `0x00000000`. O loop em `0xb0400020-0xb0400064` está iterando essa tabela tratando cada palavra como se fosse um contador de bytes-a-copiar (reinterpretação equivocada de ponteiro como length-prefix), e ao encontrar a entrada nula (índice 6) — que deveria ser um slot vazio/terminador legítimo de tabela — o cálculo produz o underflow `0xffffffff` já documentado. Hipóteses em aberto (não resolvidas): (a) a tabela deveria ter exatamente 6 entradas válidas e o loop consumidor tem um bug de off-by-one que lê 1 além do fim (nesse caso o bug é do FIRMWARE original, não do emulador); (b) a 7ª entrada deveria ter um valor não-nulo (ponteiro real) e está zerada porque uma etapa anterior do boot (registro/inicialização de serviço L4) não escreveu esse slot — nesse caso é um bug do EMULADOR (lookup L4/registro de serviço incompleto). Distinguir (a) de (b) exige comparar com uma implementação de referência do formato de tabela do `ig_naming` real (não disponível) ou rastrear TODAS as escritas em `0xb04152a0+0x18` durante o boot para ver se algo deveria tê-la preenchido e não preencheu. Nenhum fix aplicado ainda — investigação em andamento. |
