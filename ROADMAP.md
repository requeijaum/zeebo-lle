...
- **OKL4/Iguana — o mais perto de um kernel rodando**: no harness C++ com bootinfo
  populado chega ao **idle do scheduler** (`pc=0xf0002ea4`, 107 k instruções). O bloqueio
  é *ambiente* (contrato de loader/bootinfo, ABI L4e), não arqueologia de código.
...
**QW99 — 4ª hipótese do `PC=0x14` testada e REFUTADA (escrita incondicional de `R0`)**: a suspeita era que `uc_reg_write(uc, UC_ARM_REG_R0, &res_r0)` (`zeebo_lle_main.cpp:3408`), executado **antes** do bloco que trata `did_handoff`, destruísse o `R0` da thread destino — o mesmo cuidado que o código já tem com PC/SP (*"NÃO sobrescrever de volta para o wrapper IPC"*) não se estendia a `R0`. Teste RED isolado (`tools/cpp/test_pc14_r0_preserve.cpp`, com **controle positivo do instrumento**) confirmou a **mecânica**: se `res_r0 != 0`, a thread destino de fato retoma com o `R0` do chamador. **Mas a pré-condição nunca ocorre no firmware real.** Instrumentando o ramo de handoff num boot de 10 s do AppMgr: apenas **dois** handoffs em todo o boot, ambos `L4_Ipc`, e em ambos `res_r0 == 0` **e** o `R0` vivo também era `0` (`destino_pc=0xb0100000` e `0xb0300000`). A escrita incondicional é, na prática, um **no-op** — nunca corrompeu registrador algum. Aplicar `if (!did_handoff)` **não mudou o boot**: `PC=0x14` persistiu (1264 ocorrências, ~7.043.000 insns em Core0 antes e depois). Patch **revertido** (`git checkout`) — sem causa comprovada, sem patch. O teste fica no repo como refutação registrada (alvo `test-pc14-r0`, exit invertido) para impedir que a hipótese seja reaberta e "consertada" sem medição. **Próxima linha**: ligar o `stmdb sp!,{r0-r12}` de `0xb000c3d4` (que salva `r6=5` na transição de thread `0x8000c001`) ao `R0` da restauração — a origem do `5` já está localizada, falta o elo com o registrador restaurado.
