...
**QW43 — mecanismo exato do underflow confirmado (formato RLE 3-bit + escape byte)**: Análise fina do codec via disassembly linha-a-linha (`0xb0400044-0xb0400062`) mostra o layout real: o campo de 3 bits altos do control byte (`ctrl>>5`) dá a contagem de literais (0-7); se esse campo vier zero, o código NÃO trata isso como "0 literais" — ele cai no fallback de "ler um byte extra de contagem" (via `bne`/fallthrough, não `beq` — ou seja, o desvio só existe para o caso "!=0", o caso "==0" sempre lê um byte extra). Isso é consistente com um escape clássico tipo LZ (`7 = escape, lê mais`), EXCETO que aqui o campo permite valor 0 cair no mesmo caminho de escape sem diferenciar de fato "verdadeiro zero" — e o código sempre faz `subs r4,#1` no valor final (assumindo convenção 1-based: contagem real = valor lido - 1, válido apenas se valor lido >= 1). No trace real (`ZEEBO_DUMP_GUARD`/`ZEEBO_DUMP_RLE`, ambos revertidos), o BYTE EXTRA lido nessa posição específica é `0x00` — ou seja, o stream contém um valor de escape que, pela convenção 1-based do decodificador, representaria "contagem = -1" (`0-1=0xffffffff`), um estado que o codec nunca deveria produzir legitimamente (min válido seria extra_byte>=1). Duas explicações continuam em aberto sem decisão: (i) o stream de dados real na NAND está corrompido/incompleto nesse offset (ex. porque um passo ANTERIOR do boot deveria ter descomprimido/carregado mais dados nessa região de RAM e não o fez — falha de outro subsistema do emulador antes de chegar aqui); (ii) esse ponto é de fato um terminador de stream válido no formato real (ex., convenção onde extra_byte==0 sinaliza EOF) e a checagem de EOF que deveria interceptar isso ANTES da leitura ocorre em código que não foi capturado nesta janela de trace (possivelmente no dispatcher ARM em `0xb0400000-0xb0400034`, executado apenas 1-2x por chamada externa, não por iteração do loop RLE). Instrumentação de bounds check (`ZEEBO_DUMP_GUARD`, revertida) confirmou que a única comparação vista dentro da rotina (`r1+r2` computando ponteiro de fim de destino, com `r2=0x410` constante) acontece 1x por chamada externa, não por iteração — não há verificação de fim-de-ENTRADA dentro do loop capturado, reforçando (ii). Sem mais contexto de referência de formato AMSS/BREW real, distinguir (i) de (ii) com certeza exigiria uma implementação de referência do decodificador (não disponível localmente) ou uma busca de documentação pública sobre o formato de compressão usado no Zeebo/BREW/AMSS. Nenhum fix aplicado; recomendo pausa desta sub-investigação de baixo nível e avaliação de custo/benefício antes de continuar (risco de esgotar o orçamento em RE sem uma fonte de verdade externa para o formato). |
