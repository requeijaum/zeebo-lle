...
**QW43 — causa raiz localizada (byte terminador 0x00 no decodificador de string)**: instrumentação temporária (`ZEEBO_DUMP_CALLER`, revertida) confirmou que o loop `0xb0400020-0xb0400064` é um decodificador de string estilo UTF-8/length-prefixed: cada byte lido decide quantos bytes adicionais copiar (r4=2 para um byte, r4=5 para outro, etc., valores pequenos e corretos nas primeiras chamadas). Na 2ª iteração do laço externo (`0xb0400014`, `lr=0xb040001c`), o byte processado é `r3=0x00000000` (terminador de string) — mas o cálculo do decodificador NÃO trata 0x00 como caso especial de término; ele cai no mesmo cálculo genérico de "bytes de continuação" e produz `r4=0xffffffff` (underflow: provavelmente um `sub`/contagem de bits líder que espera um byte com MSB setado, e 0x00 não tem nenhum bit setado). Isso trava o loop de cópia em ~4 bilhões de iterações. Pontos ainda não decididos: (a) se isso é um BUG REAL do firmware original (Zeebo AMSS 1.1.2 nunca esperaria ver esse terminador nesse contexto — string malformada por dado incorreto vindo de estado anterior do boot), ou (b) se é um efeito colateral de algum dado/registrador que o próprio emulador está fornecendo errado antes desse ponto (ex. um comprimento de string mal calculado alimentando esse decodificador com um buffer que não deveria terminar ali). Precisa de mais RE: identificar a função chamadora (em `0xb000c3fc`/`0xb0302dd1`, vistos como LR do primeiro handoff) para saber se o buffer de entrada é dado do emulador (ex. nome de serviço L4 que construímos) ou dado genuíno do firmware. |
