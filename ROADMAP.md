...
| QW41 | **parcial (código + TDD); novo bloqueio isolado** | Core0 executa código AMSS/BREW real a partir de `0x10137000` | alto | Causa raiz identificada e corrigida em 3 pontos: (1) handoff de thread (`L4_Ipc`/`L4_ThreadSwitch`) não setava o T-bit a partir do IP alvo; (2) retorno de SVC não restaurava o T-bit quando o chamador está fora dos stubs L4 fixos (`0xb0000000-0xb0020000`, ARM) — SVC vindo de AMSS/BREW Thumb (ex. `0x103dcd18`) sempre entra em exceção ARM (hardware real) e o handler nunca restaurava o modo; (3) **achado adicional decisivo**: `uc_reg_read(PC)` no Unicorn nunca retorna o LSB setado, então a cada nova fatia (`uc_emu_start` do loop principal) o T-bit era perdido mesmo que a fatia anterior estivesse rodando em Thumb — corrigido reconstituindo o bit a partir do CPSR real antes de cada `uc_emu_start`. Prova por execução real: Core0 agora executa milhares de instruções Thumb genuínas de `0x10137000` até pelo menos `0x1039322e` (`cbz r4,...` roda como loop legítimo, não mais decode ARM inválido); em execuções subsequentes o boot avança ainda mais, travando em endereços posteriores do range de `ig_naming` (`0xb010333a`), variação atribuída a timing de scheduling cooperativo, não a regressão do fix. TDD `test_svc_thumb_resume.cpp` (RED contra CPSR-write direto/buggy; GREEN com bit0-no-PC). Suite completa + clean-hygiene verdes. **Bloqueio residual (QW42 aberto)**: boot completo do BREW Appmgr ainda não alcançado. Investigação do stall consistente em `0xb010333a` (dentro de `ig_naming`, va 0xb0100000+, CPSR=0x80000030/ARM/USR) via dump de bytes reais no ponto de travamento: os bytes ali (`ff eb f0 2f 94 e5 10 50 82 e5 ...`) NÃO decodificam como código ARM válido no offset exato (`svchs #0xf0ebff; mulspl r0,r4,r5; ...` — opcodes condicionais sem sentido, indicativo de lixo/desalinhamento) nem como Thumb alinhado; só produzem instruções plausíveis (`cmp r7,#0xf0; b ...; str r0,[r2,r0]`) quando lidos com deslocamento de +2 bytes. Isso sugere um **possível off-by-2 no PC de retomada** em algum branch/handler dentro do próprio `ig_naming`, distinto do bug de T-bit já corrigido em QW41 — ainda não localizado o ponto exato que escreve o PC errado. Log adicional: `[MMU/WARN] map_one: page @ 0x16000000/0x16800000/0x16c00000 is EMPTY/UNINITIALIZED` aparece antes do stall — merece investigação futura (pode ser normal para heap não populado, ou sintoma do mesmo problema de loader) |
