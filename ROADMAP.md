...
- [ ] **Core 0 — fechamento do BootInfo/`bi_execute` (Passo 13)**:
  ...
  - **Bloqueio atual superado (QW26, `7b570e2`)**: `ThreadControl` (0x08) e `SpaceControl` (0x18) integrados com retomada no epílogo (`pc` + `SP=ip`). Panic `SpaceControl != 1` (linha 244) foi eliminado.
  - **Próximo gate (QW27)**: Stubs `ThreadSwitch` (0x04) e `Schedule` (0x10) + desvio em direção ao escalonamento de threads / `L4_ExchangeRegisters` (0x0c) para evitar salto cego para `0x00000000`.
...
| QW27 | **em andamento** | Retomar stubs de syscall com frame `ThreadSwitch` (0x04, 0xb000c7b8) e `Schedule` (0x10, 0xb000c7cc) em `pc`+`SP=ip` e mapear despacho de threads | baixo-médio | TDD dos stubs 0x04/0x10 + investigação de despacho de PC em novas threads (L4_ExchangeRegisters 0x0c / thread entrypoint) |
