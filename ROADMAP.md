...
2. **Passo 14 — Despacho IPC no Iguana Server Loop & Boot do BREW AppMgr** (QW35 concluído, avanço para QW36)
   ...
   - QW28 a QW35 foram CONCLUÍDOS (MsgTag, ThreadTable, scheduler cooperativo em `ThreadSwitch` e `L4_Ipc`, SystemServiceRegistry, handoff AMSS/BREW, roteamento FIRSTAPP e parsing/resolução de MIFs).
   - Bloqueio atual = QW36: Instanciação do loop de execução contínua de applets/jogos comerciais sob a ponte IGL/GLES.
   - Handoff para o processo de espaço de usuário do `AEECShell` / BREW em `0x10137000` / `0x10c874f4`.
...
| QW36 | **proposto** | Despacho de ciclo de vida e renderização de jogos/applets (Double Dragon) | alto | vincular eventos de entrada e pipeline IGL/GLES na execução autônoma de jogos |
