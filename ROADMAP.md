...
**QW43 — decodificador identificado via disassembly completo (Capstone) + trace de valores reais**: Disassembly ARM/Thumb completo de `0xb0400000-0xb040007e` (dump via `ZEEBO_DUMP_CODE`, revertido) revela que a região NÃO é uma tabela simples nem um memcpy — é: (1) um dispatcher ARM em `0xb0400000-0xb0400034` que itera uma tabela de entradas de 16 bytes (`ldm sl!, {r0,r1,r2,r3}`) comparando `sl` com `fp` (fim da tabela) e chamando `bx r3` (ou `subne pc,r7,r3` se r3 ímpar) para cada entrada — a entrada nula (`r3=0`, índice 6) faz `bx 0`, o que provavelmente causaria crash, MAS o trace mostra que a execução real não passa por ali no caminho do travamento atual (a entrada nula não é a origem do loop infinito, como se pensava antes); (2) o loop travado de fato é um DECODIFICADOR RLE/LZ-like em `0xb0400038-0xb040007e` que lê um byte de controle (`ldrb r3,[r0]`), extrai dois campos via shift (r4 = literais a copiar, r5 = tamanho do back-reference), copia r4 bytes literais (loop em `0xb0400064`) e depois r5 bytes via back-reference com offset (loop em `0xb0400078`). Trace real (`ZEEBO_DUMP_RLE`, revertido) capturou os primeiros ~12 símbolos decodificados corretamente (r3/ctrl variando 0x01-0x72, r4 pequenos e válidos) até o control byte em `r0=0xb04155b8` ser `0x00` — nesse ponto o campo extra de contagem também é lido como `0x00`, produzindo `r4=0` e o subsequente `subs r4,#1` gera o underflow clássico (`0xffffffff`) que trava o loop. Questão em aberto: um control-byte `0x00` com contagem de literais zero pode ser um estado inválido do codec (bug real do fluxo de dados/streaming AMSS) OU pode ser um terminador de stream legítimo que o loop consumidor não trata (falta um check de fim-de-buffer contra `r2`/tamanho total, que permanece constante `0x00000410` nos traces — sugerindo que é um limite/tamanho e não é comparado dentro do loop capturado). Não há fix aplicado. Próximo passo: instrumentar a comparação real com `r2`/tamanho total (provavelmente feita fora da janela de código dumpada, em `0xb0400000-0xb0400034` ou por outro guard mais externo) para decidir se o consumo deveria ter parado antes de `0xb04155b8`. |
